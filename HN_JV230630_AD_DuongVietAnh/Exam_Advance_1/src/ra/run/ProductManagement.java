...
public class ProductManagement {
    ...
    private static void inputCatalogs() {
        System.out.println("----- Menu Thêm Danh Mục -----");
    ...
    private static void searchProductByCatalog() {
        ...
        for (Product product : productsList) {
            if (product.getCatalog().getCatalogName().toLowerCase().contains(catalogName.toLowerCase())) {
                product.displayData();
                found=true;
            }
        }
        if (!found) {
            System.err.println("Không tìm thấy danh mục, mời nhập tìm kiếm lại.");
        }
